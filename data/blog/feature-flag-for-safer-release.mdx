...
Sometimes the deployment went really well, but some weren't. There were cases such as **different configurations** between production and test server.
...
It's a mechanism to **turn off or on a feature** without having to do a deployment, changing env var, and anyone with access could do it, not just the developers.

It looks like an a/b test but the feature flag is not used to compare whether solution a is better than b, or trying to measure the metric, but it's more used as a **safer option to roll out feature/enhancement**.
